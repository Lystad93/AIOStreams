...
| `{stream.subtitleTranslated}` | boolean | Whether a finished AI-translated subtitle already exists for this release (or another release of the same runtime) |
| `{stream.translatedSubtitles}` | string[] | Languages an AI translation is available in, e.g. `Norwegian` |
| `{stream.translatedSubtitleCodes}` | string[] | The same, as ISO codes, e.g. `NO` |
| `{stream.translatedSubtitleEmojis}` | string[] | The same, as flag emojis, e.g. 🇳🇴 |
| `{stream.smallTranslatedSubtitleCodes}` | string[] | The same, as small-caps ISO codes |
