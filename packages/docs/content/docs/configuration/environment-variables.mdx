...
| Environment Variable                  | UI Setting                 | Type    | Default   | Description                                                                                                                                                                                                                                                                                                                                                  |
| ------------------------------------- | -------------------------- | ------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `SUBTITLE_TRANSLATION_ENABLED`        | Translation Enabled        | boolean | `true`    | Master switch for the subtitle extraction + translation feature. When disabled, no translation entries are offered to any user, regardless of their own settings.                                                                                                                                                                                            |
| `SUBTITLE_RETENTION_DAYS`             | Retention Days             | number  | `-1`      | Delete stored subtitle jobs, and their extracted/translated SRT bodies, once they are older than this many days. Each job holds two full SRT files, so on a busy instance this table grows without bound. Set -1 to keep everything forever (the default). Orphaned jobs whose user no longer exists are always removed, regardless of this setting.         |
| `SUBTITLE_DURATION_TOLERANCE_MS`      | Duration Tolerance Ms      | number  | `2000`    | How far two runtimes may differ and still count as the same cut when scoring a subtitle against the playing release. Exact equality never fires in practice — addons round differently and encodes pad the tail — so 2 seconds is the working threshold.                                                                                                     |
| `SUBTITLE_DIFF_VERBOSITY`             | Diff Verbosity             | enum    | `normal`  | How much of the difference between a subtitle and the playing release to spell out. `minimal` shows only differences that can affect sync (network, edition, revision, group, source type). `normal` adds a single marker for cosmetic differences. `full` lists every field, capped by the setting below.                                                   |
| `SUBTITLE_MAX_DIFF_TOKENS`            | Max Diff Tokens            | number  | `6`       | Hard cap on how many difference groups are rendered in `full` mode; the rest collapse to a single overflow marker.                                                                                                                                                                                                                                           |
| `SUBTITLE_MIN_DISPLAY_SCORE`          | Min Display Score          | number  | `50`      | Candidates scoring below this are computed but never shown. Raise it to keep the list short, lower it to see weak matches that only duration agreement supports.                                                                                                                                                                                             |
| `SUBTITLE_SEASON_PACK_PENALTY`        | Season Pack Penalty        | number  | `10`      | Subtracted from a whole-season subtitle that could not be resolved to the playing episode. The pack is still scored on its own release fields first, because a pack from the identical release is strong evidence the episode inside will sync.                                                                                                              |
| `SUBTITLE_DURATION_DELTA_MODE`        | Duration Delta Mode        | boolean | `false`   | Render a mismatched runtime as a signed difference (`X +3m00s`) instead of the subtitle's absolute runtime (`X 1h53m28s`). Shorter, and usually easier to judge at a glance.                                                                                                                                                                                 |
| `SUBTITLE_UNICODE_ARROW`              | Unicode Arrow              | boolean | `false`   | Render translation direction as `NOR←ENG` instead of `NOR<ENG`. Off by default because the arrow does not render reliably across all Stremio clients.                                                                                                                                                                                                        |
| `SUBTITLE_EXTRACTION_TIMEOUT_SECONDS` | Extraction Timeout Seconds | number  | `1800`    | How long ffmpeg may spend demuxing one release before the job is failed. Extraction streams the file over the network, so a large remux on a slow backbone legitimately takes far longer than a local copy would — the old 5-minute ceiling failed 20 GB files that were progressing normally. Raise it if you see "ffmpeg timed out" on big releases.       |
| `SUBTITLE_EXTRACTION_ALLOWED`         | Extraction Allowed         | boolean | `true`    | Allow the full-file extraction path, which streams the release to demux its embedded subtitle track. On a public multi-tenant instance you may want this off, leaving only features that need no download. Users must supply their own translation API key either way.                                                                                       |
| `SUBTITLE_SHARE_TRANSLATIONS`         | Share Translations         | boolean | `true`    | Let any user be served a finished translation another user already paid for, when it is for the same release and the same target language. Translation is by far the expensive step, so re-running it per user wastes both time and API quota for an identical result. Turn off to keep each user's translations private to them.                            |
| `SUBTITLE_SHARE_SOURCES`              | Share Sources              | boolean | `true`    | An extracted subtitle depends only on the release, not on who requested it or which language they were translating into. Sharing lets a second user — or the same user picking a different target language — reuse an existing extraction instead of re-downloading and re-demuxing the whole file. Disable to keep each user’s extractions private to them. |
| `SUBTITLE_EXTERNAL_ENABLED`           | External Enabled           | boolean | `true`    | Look up subtitles for the exact release being played on the configured providers (SubSource, SubDL). Matches are offered alongside the extraction entry with a confidence percentage, and a good match can be translated without downloading the video at all. Needs at least one provider API key below.                                                    |
| `SUBTITLE_SUBSOURCE_API_KEY`          | Subsource API Key          | string  | _(unset)_ | API key for [SubSource](https://subsource.net/api-docs). Its subtitles list every release they are synced to, which gives the most reliable matches. _(secret)_                                                                                                                                                                                              |
| `SUBTITLE_SUBDL_API_KEY`              | Subdl API Key              | string  | _(unset)_ | API key for [SubDL](https://subdl.com/panel/api). Supports searching by the exact release filename. _(secret)_                                                                                                                                                                                                                                               |
| `SUBTITLE_EXTERNAL_USE_LIMIT`         | External Use Limit         | number  | `3`       | How many matched external subtitles to list for playing directly, best match first. Set 0 to hide them and leave only the translate entries.                                                                                                                                                                                                                 |
| `SUBTITLE_EXTERNAL_TRANSLATE_LIMIT`   | External Translate Limit   | number  | `3`       | How many matched external subtitles to offer translating into your target language. Counted separately from the entries above, so you can list several to use but only translate the best one. Set 0 to hide them.                                                                                                                                           |
| `SUBTITLE_EXTERNAL_CACHE_SIZE`        | External Cache Size        | number  | `500`     | How many downloaded external subtitle files to keep, so re-selecting one does not re-download and re-unpack it. This cache is entirely separate from extracted and translated subtitles — those are stored permanently in the database and are never evicted by this limit. Set 0 to disable caching. _(restart required)_                                   |
| `SUBTITLE_OPENSUBTITLES_API_KEY`      | Opensubtitles API Key      | string  | _(unset)_ | App key for [OpenSubtitles](https://www.opensubtitles.com/en/consumers). This is enough to SEARCH. Downloading counts against a personal daily quota, so each user must also supply their own OpenSubtitles username and password in their settings — the instance key is never used to spend someone else’s quota. _(secret)_                               |
| `SUBTITLE_DURATION_TOLERANCE_SECONDS` | Duration Tolerance Seconds | number  | `60`      | How far two runtimes may differ and still count as the same content. Applied as `max(this, percentage below)`: a fixed floor absorbs the minute-level rounding in addon-reported runtimes, while the percentage scales with long films.                                                                                                                      |
| `SUBTITLE_DURATION_TOLERANCE_PERCENT` | Duration Tolerance Percent | number  | `0.5`     | Percentage of the runtime allowed to differ, used when it is larger than the fixed tolerance above.                                                                                                                                                                                                                                                          |
| `SUBTITLE_REUPLOAD_TAGS`              | Reupload Tags              | list    | —         | Extra suffixes that re-hosting sites append to an existing release name (e.g. **wtf** for `…-Kitsune-WtF`). These are ignored when matching a stored subtitle against the release being played, so a re-upload still finds it. **wtf** is recognised by default.                                                                                             |
